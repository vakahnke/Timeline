...
const url = {
  ...
  templates:  ()        => '/templates/',
  // Template keys look like "saved:12" / "builtin:sprint".
  template:   (id)      => `/templates/${encodeURIComponent(id)}/`,
  templateDo: (id, what) => `/templates/${encodeURIComponent(id)}/${what}/`,
  templateComment: (id, cid) => `/templates/${encodeURIComponent(id)}/comments/${cid}/`,
  templateNote:    (id, nid) => `/templates/${encodeURIComponent(id)}/owner-notes/${nid}/`,
  templateLesson:  (id, lid) => `/templates/${encodeURIComponent(id)}/lessons-learned/${lid}/remove/`,
...
export const api = {
  ...
  templates: {
    list:        (params) => req(url.templates() + (params ? `?${new URLSearchParams(params)}` : '')),
    get:         (key)    => req(url.template(key)),
    update:      (key, d) => req(url.template(key), { method: 'PATCH', body: body(d) }),
    ...
    // Lessons learned. Every write answers with the whole list, so the page just replaces it.
    lessonsLearned: (key)         => req(url.templateDo(key, 'lessons-learned')),
    removeLesson:   (key, lid)    => req(url.templateLesson(key, lid), { method: 'POST' }),
    addNote:        (key, d)      => req(url.templateDo(key, 'owner-notes'), { method: 'POST', body: body(d) }),
    updateNote:     (key, nid, d) => req(url.templateNote(key, nid), { method: 'PATCH', body: body(d) }),
    removeNote:     (key, nid)    => req(url.templateNote(key, nid), { method: 'DELETE' }),
    ...
    updateComment:  (key, cid, d) => req(url.templateComment(key, cid), { method: 'PATCH', body: body(d) }),
    removeComment:  (key, cid)    => req(url.templateComment(key, cid), { method: 'DELETE' }),

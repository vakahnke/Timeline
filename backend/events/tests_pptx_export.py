...
class PptxContentTests(_Base):
    ...
    def test_the_products_name_comes_from_the_setting(self):
        from django.test import override_settings
        with override_settings(APP_NAME='Harbour Test Name'):
            slide = self.build().slides[0]
        self.assertIn('Generated by Harbour Test Name;', slide.notes_slide.notes_text_frame.text)
        source = next(sh for sh in slide.shapes if sh.name == 'Source')
        self.assertEqual(source.text_frame.text, 'Harbour Test Name')
